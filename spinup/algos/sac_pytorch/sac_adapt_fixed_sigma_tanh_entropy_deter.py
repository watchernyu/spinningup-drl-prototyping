...
def sac_adapt(env_fn, hidden_sizes=[256, 256], seed=0,
    ...
    entropy_net = nn.Sequential(nn.Linear(2,32),
                        nn.ReLU(),
                        nn.Linear(32,32),
                        nn.ReLU(),
                        nn.Linear(32,1))
    ...
    for t in range(total_steps):
        ...
        if use_one_step_version and replay_buffer.size >= batch_size:
            ...
            """get q loss"""
            with torch.no_grad():
                a_tilda_next, _, _, log_prob_a_tilda_next, _, _, tanh_entropy = policy_net.forward(obs_next_tensor, deterministic=True, fixed_sigma=True, batch_size=batch_size, tanh_entropy=True, entropy_net=entropy_net)
                ...
                min_next_q = torch.min(q1_next, q2_next) + alpha * tanh_entropy
            ...
            get policy loss
            """
            a_tilda, mean_a_tilda, log_std_a_tilda, log_prob_a_tilda, _, _, tanh_entropy = policy_net.forward(obs_tensor, deterministic=True, fixed_sigma=True, batch_size=batch_size, tanh_entropy=True, entropy_net=entropy_net)
            ...
            policy_loss = ( - alpha * tanh_entropy - min_q1_q2_a_tilda).mean()
            ...
            """
            if auto_alpha:
                alpha_loss = -(log_alpha * (-tanh_entropy + target_entropy).detach()).mean()
