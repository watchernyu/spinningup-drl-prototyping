...
def sac_adapt(env_fn, hidden_sizes=[256, 256], seed=0,
              ...
              polyak=0.995, lr=3e-4, alpha=0.2, penal_alpha = 0.1, batch_size=256, start_steps=10000,
    ...
    for t in range(total_steps):
        ...
        if use_one_step_version and replay_buffer.size >= batch_size:
            ...
            """get q loss"""
            with torch.no_grad():
                a_tilda_next, _, _, log_prob_a_tilda_next, _, _, penal_a = policy_net.forward(obs_next_tensor, penal_a=True)
                ...
                y_q = rews_tensor + gamma * (1 - done_tensor) * min_next_q + penal_alpha*penal_a
            ...
            get policy loss
            """
            a_tilda, mean_a_tilda, log_std_a_tilda, log_prob_a_tilda, _, _, penal_a = policy_net.forward(obs_tensor, penal_a=True)
