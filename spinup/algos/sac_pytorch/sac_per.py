...
def sac_pytorch_per(env_fn, hidden_sizes=[256, 256], seed=0,
                    ...
                    PER_alpha=0.6, PER_beta_start=0.6, use_v_in_td=False, use_value_policy_is_weight=False):
    """
    ...
    for t in range(total_steps):
        ...
        if d or (ep_len == max_ep_len):
            ...
            for j in range(ep_len):
                ...
                ## TODO maybe should try also multiply weights with value and policy loss
                ...
                if not use_v_in_td:
                    ...
                    # here we compute absolute TD error to be the average of abs TD error of 2 q networks and value net
                    abs_td = ((q1_prediction.detach() - y_q.detach()).abs() +
                              (q2_prediction.detach() - y_q.detach()).abs() +
                              (v_prediction.detach()  - y_v.detach()).abs()) / 3

                if not use_value_policy_is_weight: # same as vanilla
...
if __name__ == '__main__':
    ...
    sac_pytorch_per(lambda: gym.make(args.env), hidden_sizes=[args.hid] * args.l,
                    gamma=args.gamma, seed=args.seed, epochs=args.epochs,
                    steps_per_epoch=args.steps_per_epoch,
                    logger_kwargs=logger_kwargs)

    # ##TODO debugging in progress
    # memory = PrioritizedReplayMemory(10)
    # memory.push([1,2])
    # memory.push([2,3])
    # memory.push([4,5])
    # memory.push([6,7])
