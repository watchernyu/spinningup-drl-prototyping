...
def sac_adapt(env_fn, hidden_sizes=[256, 256], seed=0,
              ...
              polyak=0.995, lr=3e-4, alpha=0, b=3, batch_size=256, start_steps=10000,
    ...
    def test_agent(n=5):
        ...
        for j in range(n):
            ...
            while not (d or (ep_len == max_ep_len)):
                ...
                a = policy_net.get_env_action(o, deterministic=True, fixed_sigma=True, clip_action=True, b=b)
    ...
    for t in range(total_steps):
        ...
        """
        if t > start_steps:
            a = policy_net.get_env_action(o, deterministic=False, fixed_sigma=True, clip_action=True, b=b)
        else:
        ...
        if use_one_step_version and replay_buffer.size >= batch_size:
            ...
            """get q loss"""
            with torch.no_grad():
                a_tilda_next, _, _, log_prob_a_tilda_next, _, _ = policy_net.forward(obs_next_tensor, fixed_sigma=True, clip_action=True, b=b)
            ...
            get policy loss
            """
            a_tilda, mean_a_tilda, log_std_a_tilda, log_prob_a_tilda, _, _ = policy_net.forward(obs_tensor, fixed_sigma=True, deterministic=True, clip_action=True, b=b)
