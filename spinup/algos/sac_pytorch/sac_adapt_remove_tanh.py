...
def sac_adapt(env_fn, hidden_sizes=[256, 256], seed=0,
    ...
    def test_agent(n=5):
        ...
        for j in range(n):
            ...
            while not (d or (ep_len == max_ep_len)):
                ...
                a = policy_net.get_env_action(o, deterministic=True, removed_tanh=True)
                print (a)
    ...
    for t in range(total_steps):
        ...
        hhhh = 0
        if t > start_steps:
            a = policy_net.get_env_action(o, deterministic=False, removed_tanh=True)
            #if hhhh == 0:
            print (a)
            #    hhhh += 1
        else:
        ...
        if use_one_step_version and replay_buffer.size >= batch_size:
            ...
            """get q loss"""
            with torch.no_grad():
                a_tilda_next, _, _, log_prob_a_tilda_next, _, pre_tanh_value = policy_net.forward(obs_next_tensor, removed_tanh=True)
                #if removed_tanh:
                #    a_tilda_next = pre_tanh_value
            ...
            get policy loss
            """
            a_tilda, mean_a_tilda, log_std_a_tilda, log_prob_a_tilda, _, pre_tanh_value = policy_net.forward(obs_tensor, removed_tanh=True)
            #if removed_tanh:
            #    a_tilda = pre_tanh_value
