...
def sac_adapt(env_fn, hidden_sizes=[256, 256], seed=0,
    ...
    for t in range(total_steps):
        ...
        if use_one_step_version and replay_buffer.size >= batch_size:
            ...
            """get q loss"""
            with torch.no_grad():
                a_tilda_next, _, _, log_prob_a_tilda_next, _, _, mu_l4 = policy_net.forward(obs_next_tensor, mu_l4=True)
                ...
                min_next_q = torch.min(q1_next, q2_next) + alpha * mu_l4
            ...
            get policy loss
            """
            a_tilda, mean_a_tilda, log_std_a_tilda, log_prob_a_tilda, _, _, mu_l4 = policy_net.forward(obs_tensor, mu_l4=True)
            ...
            policy_loss = ( - alpha * mu_l4 - min_q1_q2_a_tilda).mean()
            ...
            """
            if auto_alpha:
                alpha_loss = -(log_alpha * (-mu_l4 + target_entropy).detach()).mean()
