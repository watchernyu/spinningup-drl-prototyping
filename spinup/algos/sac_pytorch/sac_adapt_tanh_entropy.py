...
def sac_adapt(env_fn, hidden_sizes=[256, 256], seed=0,
    ...
    for t in range(total_steps):
        ...
        if use_one_step_version and replay_buffer.size >= batch_size:
            ...
            """get q loss"""
            with torch.no_grad():
                a_tilda_next, _, _, log_prob_a_tilda_next, _, _, tanh_entropy = policy_net.forward(obs_next_tensor, batch_size=batch_size, tanh_entropy=True, entropy_net=entropy_net)
            ...
            get policy loss
            """
            a_tilda, mean_a_tilda, log_std_a_tilda, log_prob_a_tilda, _, _, tanh_entropy = policy_net.forward(obs_tensor, batch_size=batch_size, tanh_entropy=True, entropy_net=entropy_net)
