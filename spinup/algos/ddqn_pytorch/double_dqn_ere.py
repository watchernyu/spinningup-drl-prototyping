...
class EfficientReplayBuffer:
    ...
    def sample_priority_only_batch(self, force_priority_count, batch_size=32):
        ...
        recent_relative_idxs = np.random.randint(0, max_index, size=recent_data_size) * (-1)

        for i in range(len(recent_relative_idxs)):
            absolute_index = (self.ptr-1 + recent_relative_idxs[i])%self.size
            while self.current_state_done_buf[absolute_index] == 1:
                # if == 1 means this state is a terminal state
                # then need to resample this one
                recent_relative_idxs[i] = -np.random.randint(0, max_index)
                absolute_index = (self.ptr - 1 + recent_relative_idxs[i]) % self.size

        recent_idxs = (self.ptr-1 + recent_relative_idxs)%self.size
        next_state_idxs = (recent_idxs + 1) % self.size

        return dict(obs1=self.obs_buf[recent_idxs],
                    obs2=self.obs_buf[next_state_idxs],
...
def double_dqn_ere(env_id ="PongNoFrameskip-v4", seed = 0, buffer_size = int(1e5), replay_initial = 50000,
                   ...
                   eta_0=0.996, c_min=5000, eta_final=1.0, no_eta_anneal=False,
                   logger_kwargs=dict()):
    # ere set up
    if no_eta_anneal:
        eta_final = eta_0
    ...
    for step_idx in range(1, num_step + 1):
        ...
        if done and len(replay_buffer) >= replay_initial:
            ...
            eta_current = compute_current_eta(eta_0, eta_final, step_idx, num_step)
            ...
            for k in range(num_updates):
                ...
                loss = huber_criterion(q_value, Variable(expected_q_value.data))
...
if __name__ == '__main__':
    ...
    print("program start")
    sys.stdout.flush()
    double_dqn_ere(env_id=args.env, seed=args.seed, buffer_size=args.b_size, replay_initial=args.replay_init, num_step=args.steps,
